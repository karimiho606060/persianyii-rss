...
class Rss extends \yii\web\Widget {
    var $rss_version = '2.0';
    var $encoding = 'utf-8';
    var $stylesheet = '';
    ...
    function createFeed($channel)    {
        ...
        if(array_key_exists('description', $textInput))
            $rss .= '      <title>' . $textInput['description'] . '</title>' . "\n";
           if(array_key_exists('name', $textInput)){
            $rss .= '      <title>' . $textInput['name'] . '</title>' . "\n";

               if(array_key_exists('link', $textInput))
            $rss .= '      <title>' . $textInput['link'] . '</title>' . "\n";
        ...
        foreach ($channel['items'] as $item) {
            ...
            if (array_key_exists('source',$item)) {
                if (array_key_exists('source url',$item))
